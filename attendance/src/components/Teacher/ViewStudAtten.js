...
const ViewStudAtten = () => {
    ...
    }, [student, subj, navigate]);

    if (!student || !subj) {
        return null;
    }
    const beginDate = attendance.length
        ? new Date(attendance[attendance.length - 1].date).toLocaleDateString('en-GB')
        : '-';
                ...
                .then(function (response) {
                    ...
                    if (response.status === 203) {
                        ...
                        setattendance((prev) => prev.map((entry) => {
                            if (entry.id === updId) {
                                return { ...entry, status: atendstatus };
                            }
                            return entry;
                        }));
                    }
